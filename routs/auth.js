...
router.post("/log_in", async (req, res) => {
    if (!helper.check_inputs(["user_name", "password"], req.body)) return res_handler.failed(res, "INVALID_INPUTS")
    ...
    if (!active)return  res_handler.failed(res, "BLOCKED_USER")
...
router.post("/forget_password", async (req, res) => {
    if (!helper.check_inputs(["phone"], req.body)) return res_handler.failed(res, "INVALID_INPUTS")
    ...
    if (cant_send) return res_handler.failed(res, "NEED_TO_WAIT")
    if (!selectd_user) return res_handler.failed(res, "INVALID_USER")
...
router.post("/change_password_session", async (req, res) => {
    if (!helper.check_inputs(["session_id", "new_password"], req.body)) return res_handler.failed(res, "INVALID_INPUTS")
    const { session_id, new_password } = req.body
    const selected_session = await ForgetPassword.findOne({ session_id, used: false })
    if (!selected_session) return res_handler.failed(res, "INVALID_SESSION")
